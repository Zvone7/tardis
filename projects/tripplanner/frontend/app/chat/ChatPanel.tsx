...
import { Send, Square, ImagePlus, X, ArrowDown, MessageSquarePlus, History, Trash2, Mic, MicOff, Minus, MessageCircle } from "lucide-react"
...
export function ChatPanel({ mode = "overlay" }: { mode?: "overlay" | "inline" }) {
  const {
    tripId,
    tripName,
    messages,
...
const readFileAsDataUrl = useCallback((file: File): Promise<string> => {
  ...
  const panelBody = (
    <>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b shrink-0">
        <div>
          <p className="text-base font-semibold">Trip Assistant</p>
          {tripName && (
      ...
      {/* Session list */}
      {showSessions && (
        <div className="border-b max-h-[200px] overflow-y-auto shrink-0">
  ...
  // Inline mode: always render as flex column (visibility controlled by parent grid)
  if (mode === "inline") {
    return (
  ...
  // Wide screens: minimized pill
  if (isWide && isOpen && isMinimized) {
    return (
      <button
        onClick={() => setIsMinimized(false)}
        className="fixed bottom-20 left-6 z-50 flex items-center gap-2 rounded-full bg-primary text-primary-foreground shadow-lg px-4 py-3 text-sm font-medium hover:bg-primary/90 transition-colors"
      >
  ...
  // Wide screens: plain fixed div — no Radix, no DismissableLayer, no interference with modals
  if (isWide) {
    if (!isOpen) return null
    return (
      <div className="fixed inset-y-0 left-0 z-40 flex flex-col w-[400px] bg-background border-r shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out">
        {panelBody}
      </div>
    )
  }
